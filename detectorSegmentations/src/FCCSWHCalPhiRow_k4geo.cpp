...
namespace DDSegmentation {
  ...
  std::vector<FCCSWHCalPhiRow_k4geo::LayerInfo> FCCSWHCalPhiRow_k4geo::initLayerInfo() const {
    ...
    for (uint i_section = 0; i_section < m_offsetZ.size(); i_section++) {
      ...
      // In the li.groupedRows we store the numbers extracted from m_groupedRows vector provided from the xml file.
      // The number of non-zero elements in li.groupedRows corresponds to the number of pseudo-layers in i_section.
      // Each non-zero element in li.groupedRows corresponds to the number of rows form the physical layer of i_section
      ...
      for (uint i_dR = 0; i_dR < N_dR; i_dR++) {
        ...
        for (int i_lay = 0; i_lay < m_numLayers[i_dR + i_section * N_dR]; i_lay++) {
          ...
          out.push_back(LayerInfo{.radius = moduleDepth[i_section] - m_dRlayer[i_dR] * 0.5,
                                  ...
                                  .zmin = zmin,
                                  .zmax = zmax,
                                  .groupedRows = std::span<const int>(std::to_address(groupedRows_start), std::to_address(groupedRows_end))
                                 });
        }
      }
    }
  ...
  void FCCSWHCalPhiRow_k4geo::defineCellIndexes(LayerInfo& li, const uint layer) const {
    ...
    while ((minLayerZ + (irow + 1) * m_dz_row) < (maxLayerZ + 0.0001)) {
      ...
      {
        int nrows = 0;
        for(unsigned int i = 0; i < li.groupedRows.size(); i++)
        {
          nrows += li.groupedRows[i];
          if (irow < nrows)
          {
